...
app.get("/repo/:repoId/current", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    ...
    const apiResponse = await (0, repo_1.renderApiReponse)(repoId, datasource, renderedState, applicationState, repoState);
    if (!apiResponse) {
        res.sendStatus(404);
        return;
    }
...
app.get("/repo/:repoId/sourcegraph", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        const sourceGraphResponse = await (0, repo_1.renderSourceGraphInputs)(repoId, datasource);
...
app.post("/repo/:repoId/comparison", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    ...
    const repoState = await (0, repo_1.updateComparison)(datasource, repoId, against, branch, sha);
    if (!repoState) {
        res.sendStatus(400);
        return;
    }
...
app.post("/repo/:repoId/commit", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    ...
    const commitData = await (0, repoapi_1.writeRepoCommit)(datasource, repoId, message);
    if (!commitData) {
        res.sendStatus(400);
        return;
    }
    const [repoState, renderedState] = await Promise.all([
        (0, repo_1.changeCommandMode)(datasource, repoId, "view"),
        (0, repo_1.getApplicationState)(datasource, repoId),
    ]);
    const applicationState = await (0, repo_1.convertRenderedCommitStateToKv)(datasource, renderedState);
    const apiResponse = await (0, repo_1.renderApiReponse)(repoId, datasource, renderedState, applicationState, repoState);
...
app.get("/repo/:repoId/sha/:sha/canswitchwip", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        const canSwitch = await (0, repoapi_1.canSwitchShasWithWIP)(datasource, repoId, sha);
        if (canSwitch == null) {
            res.sendStatus(400);
            return;
        }
...
app.get("/repo/:repoId/sha/:sha/canautomerge", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    const sha = req.params["sha"];
    try {
        const [canAutoMergeOnTopOfCurrentState, canAutoMergeOnUnStagedState,] = await Promise.all([
            (0, repo_1.getCanAutoMergeOnTopCurrentState)(datasource, repoId, sha),
            (0, repo_1.getCanAutoMergeOnUnStagedState)(datasource, repoId, sha),
        ]);
...
app.post("/repo/:repoId/sha/:sha/merge", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    const sha = req.params["sha"];
    if (!repoId || !sha) {
        res.sendStatus(404);
    ...
    try {
        const renderedState = await (0, repoapi_1.mergeCommit)(datasource, repoId, sha);
...
app.post("/repo/:repoId/merge/abort", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        const renderedState = await (0, repoapi_1.abortMerge)(datasource, repoId);
        const repoState = await datasource.readCurrentRepoState(repoId);
        if (!renderedState) {
            res.sendStatus(400);
            return;
...
app.post("/repo/:repoId/merge/resolve", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        const renderedState = await (0, repoapi_1.resolveMerge)(datasource, repoId);
        const repoState = await datasource.readCurrentRepoState(repoId);
        if (!renderedState) {
            res.sendStatus(400);
            return;
...
app.post("/repo/:repoId/merge/direction/:direction", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    ...
    try {
        ...
        const renderedState = await (0, repoapi_1.updateMergeDirection)(datasource, repoId, direction);
        const repoState = await datasource.readCurrentRepoState(repoId);
        if (!renderedState) {
            res.sendStatus(400);
            return;
...
app.post("/repo/:repoId/stash", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    const renderedState = await (0, repoapi_1.stashChanges)(datasource, repoId);
...
app.post("/repo/:repoId/popstash", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    const renderedState = await (0, repoapi_1.popStashedChanges)(datasource, repoId);
    const [repoState, applicationState] = await Promise.all([
        datasource.readCurrentRepoState(repoId),
        (0, repo_1.convertRenderedCommitStateToKv)(datasource, renderedState),
    ]);
...
app.post("/repo/:repoId/discard", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    const renderedState = await (0, repoapi_1.discardCurrentChanges)(datasource, repoId);
    const [repoState, applicationState] = await Promise.all([
        datasource.readCurrentRepoState(repoId),
        (0, repo_1.convertRenderedCommitStateToKv)(datasource, renderedState),
    ]);
...
app.post("/repo/:repoId/branch", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        if (switchBranchOnCreate && branchHead) {
            const canSwitch = await (0, repoapi_1.canSwitchShasWithWIP)(datasource, repoId, branchHead);
            if (!canSwitch) {
                res.sendStatus(400);
                return;
            }
        }
        const repoState = await (0, repoapi_1.createRepoBranch)(datasource, repoId, branchName, branchHead, baseBranchId, switchBranchOnCreate);
        if (repoState == null) {
            res.sendStatus(400);
            return;
        }
...
app.post("/repo/:repoId/branch/update", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    const baseBranchId = req.body["baseBranchId"] ?? null;
    try {
        const repoState = await (0, repoapi_1.updateLocalBranch)(datasource, repoId, branchName, branchHead, baseBranchId);
        if (repoState == null) {
            res.sendStatus(400);
            return;
        }
...
app.post("/repo/:repoId/branch/switch", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        const canSwitch = await (0, repoapi_1.canSwitchShasWithWIP)(datasource, repoId, branch?.lastCommit);
        if (!canSwitch) {
            res.sendStatus(400);
            return;
        }
        const repoState = await (0, repoapi_1.switchRepoBranch)(datasource, repoId, branch?.id);
        if (repoState == null) {
            res.sendStatus(400);
            return;
        }
...
app.post("/repo/:repoId/branch/:branchId/delete", (0, cors_1.default)(corsOptionsDelegate), async (req, res) => {
    const repoId = req.params["repoId"];
    if (!repoId) {
        res.sendStatus(404);
        return;
    ...
    try {
        ...
        const repoState = await (0, repoapi_1.deleteLocalBranch)(datasource, repoId, branch?.id);
        if (repoState == null) {
            res.sendStatus(400);
            return;
        }
